...
class PCA:
    ...
    def __init__(self, path, components=10, full_spectrum=True):
        ...
        self.df = pd.read_csv(self.csv_filename, low_memory=False) #enter file name
        ...
        self.output_filename = SCORES_DIR + '/' + self.input_filename.split('/')[-1] + '-SCORES.xlsx'
    ...
    def run(self):
        ...
        #mean centering:   
        average = dataf.mean(axis=0).values #find average of each column for mean centering (array includes class average as well in first column position)
        ...
        principalComponents = pca.fit_transform(x_mc) #change variable in brackets
        ...
        loadings = pd.DataFrame(pca.components_.T, columns=['PC' + str(i) for i in range(1, self.num_components + 1)], index=lines) #find loadings for each wavelength and corresponding PC
        print("Loadings:")
        print(loadings)
        '''
        plt.figure()
